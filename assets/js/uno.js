...
function initConfig(){
    ...
    if(res.hasOwnProperty('menu')) {
        ...
        if (res.telegram.account) {
            ...
            $('.panel-twitter .block-title h2').html(res.telegram.title);
            $('.panel-twitter .author strong').html('@' + res.telegram.account);
            $('.panel-twitter .tw-description').html(res.telegram.description);
            //initTwitterAPI();
        }
